...
| **區域輪替** | 依照 `area_keyword` 設定的順序循環找票，全部沒票時不 reload 直接重新循環 |
...
- ✅ 關鍵字解析改用 `util.parse_keyword_string_to_array()`（修正引號殘留問題）
- ✅ `<area>` 座位圖載入時機：Phase 3 改為只呼叫 `fnSeatUpdate()`，移除重複的 `fnCheck()` 呼叫
- ✅ 區域輪替：點擊區域後在同一迴圈內確認是否有可購買座位，無票則自動跳下一區域
- ✅ 全部區域無票時改用 `fnSeatUpdate()` 重整座位圖（避免 page reload 導致卡死）
